wetbulb_intersect <- function(temp.wb, alt = 0) {
  ...
  #'
  #' Calculates the drybulb temperature at which the wetbulb temperature line intersects in the psychrometric chart.
  ...
  #' @return Returns a vector of dry-bulb temperatures at which the wet-bulb temperature line intersects [degC].
  #' @keywords internal
  #' @export
  #' @examples
  ...
  W_s <- sat_hum_ratio(temp.wb, alt)
  
  # Return dry-bulb temperature for given wet-bulb temperature (Eq. 35)
  return(-2.36581 * ((temp.wb - 1050.84) * W_s - 0.422689 * temp.wb))
}