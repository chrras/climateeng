pmv <- function(clo, temp.air, temp.rad, rel.hum, met = 1.2, wme = 0,
                air.velo = 0.1, par.w.press = 0) {
  #' Predicted mean vote (pmv)
  #'
  #' Vectorized calculation of the predicted mean vote (PMV) according to
  #'  ASHRAE 55-2010.
  ...
  #'  0.1 m/s.
  #' @param par.w.press Vector of water vapour pressures in [Pa]. Defaults to
  #'  0 Pa.
  ...
  sat.w.press <- exp(16.6536 - 4030.183 / (temp.air + 235))
  ...
  HL1 <- 3.05 * 0.001 * (5733 - 6.99 * MW - par.w.press) # Heat loss from skin
